...
export async function GET(
  ...
  try {
    ...
    const dateIsValid = /^(0[1-9]|[12][0-9]|3[01])-(0[1-9]|1[0-2])-\d{4}$/.test(
      date,
    );

    if (!dateIsValid) throw new Error("Invalid Date");
    ...
    const [needWaterPlants, needFertilizerPlants] = await Promise.all([
    ...
    for (const plant of needWaterPlants) {
      plantsInNeedMap.set(plant.id, { ...plant });
    }

    for (const plant of needFertilizerPlants) {
